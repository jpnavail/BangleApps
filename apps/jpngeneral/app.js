...
var Anton;
Anton = require("Storage").read("P_Anton.raw");
...
var Anton_Small;
Anton_Small = require("Storage").read("P_Anton_Small.raw");
...
function Sleep(s) { var nb; 
  if (Math.floor(s) != 0) {
    for (nb = 1; nb <= Math.floor(s); nb++) Tsleep(1000);
  }
  nb = Math.floor(s) - s;
  console.log(nb);
  if (nb != 0)  Tsleep(nb);
...
function color_it(c_champ) {
  var ic;
  g.setColor(Number("0x000F"));
  for (ic in couleur) {
    if (c_champ == couleur[ic].nom) {
      g.setColor(Number(couleur[ic].valeur));
      ic = couleur.length;
    }
  }
}
...
function lit_fic(i) {
  console.log(fic = fic_img[i] + ".raw", "Lecture");
  img = require("Storage").read(fic = fic_img[i] + ".raw");
